...
# 한글이 없는 경로로 변경
cache_dir = os.path.join('C:\\', 'leekyeongwuk', 'vector_cache')
...
if os.path.exists(cache_file):
    ...
    print(f"캐시된 벡터 저장소 업로드 완료: {time.time() - load_start:.2f}초")
else:
...
검색된 다음 문맥(context) 을 사용��여 질문(question) 에 답하세요. 만약, 주어진 문맥(context) 에서 답을 찾을 수 없다면, 답을 모른다면 `주어진 정보에서 질문에 대한 정보를 찾을 수 없습니다` 라고 답하세요.

...
data_path = os.path.join(parent_dir, 'back', 'mediinfo.txt')
...
prompt = PromptTemplate.from_template(
    """당신은 질문-답변(Question-Answering)을 수행하는 친절한 AI 어시스턴트입니다. 당신의 임무는 주어진 문맥(context) 에서 주어진 질문(question) 에 답하는 것입니다. 검색된 다음 문맥(context) 을 사용하여 질문(question) 에 답하세요. 만약, 주어진 문맥(context) 에서 답을 찾을 수 없다면, 답을 모른다면 `주어진 정보에서 질문에 대한 정보를 찾을 수 없습니다` 라고 답하세요. 한글로 답변해 주세요. 단, 기술적인 용어나 이름은 번역하지 않고 그대로 사용해 주세요.
...
recieved_question = "뉴테란 캡슐 200mg의 효능을 알려주세요."
...
# recieved_question = sys.argv[1]
recieved_question = "게보린의 사용법은?"
answer = rag_chain.invoke(recieved_question)
print(answer)
print(f"질문 처리 완료: {time.time() - query_start:.2f}초")
print(f"전체 실행 시간: {time.time() - start_time:.2f}초")
>>>>>>> 1c615e009e6b65dce0dc58f6eb72dbda7c567870
